...
import { MAX_SCALE } from '@/utils/constant';

const MAX_SCALE_PERCENTAGE = MAX_SCALE * 100;
const DEFAULT_PERCENT_OPTIONS = Array.from(
  { length: 8 },
  (_, i) => MAX_SCALE_PERCENTAGE - i * 100
).concat([75, 50, 25, 10]);
const unit = MAX_SCALE_PERCENTAGE / 100;
...
function PercentSelect({
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val === '' || /^\d*$/.test(val)) {
      const num = min([parseInt(val, 10), MAX_SCALE_PERCENTAGE]);
      setInputValue(num === undefined ? value.toString() : num.toString());
    }
  };

  const handleInputBlur = () => {
    const num = min([Math.round(parseFloat(inputValue)), MAX_SCALE_PERCENTAGE]);
